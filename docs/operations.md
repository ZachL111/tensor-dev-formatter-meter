...
`tensor-dev-formatter-meter` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies formatter behavior through fixture event logs, with golden state snapshots and no network dependency.
...
The score is deterministic. The current threshold is 162. Risk has a penalty of 7, latency has a penalty of 4, and weight contributes a bonus of 3.
